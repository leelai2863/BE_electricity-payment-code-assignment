...
| `GIT_PULL_TOKEN` | Tuỳ chọn | Nếu **không** set, workflow dùng **`GITHUB_TOKEN`** của job để `git pull` (repo private vẫn được). Có thể thêm PAT classic (`repo`) nếu muốn token dài hạn; workflow ưu tiên secret này hơn `GITHUB_TOKEN`. |
